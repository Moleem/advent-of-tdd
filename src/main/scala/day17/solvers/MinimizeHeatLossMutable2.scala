...
object MinimizeHeatLossMutable2 extends ProblemSolver[String, Int] {
    ...
    def getMinHeatLossMap: Map[CacheKey, Int] = {
      if (remainingSteps.isEmpty) {
        ...
        if (smallestKnownHeatLosses.contains(key)) {
          ...
          val allowedNeighbors =
            if (head.straightStepsSoFar < 4) {
              existingNeighbors.filter { case (direction, coordinate) => head.prevDirection.isEmpty || head.prevDirection.contains(direction)}
            } else if (head.straightStepsSoFar < 10) {
              existingNeighbors.filter { case (direction, coordinate) => !head.prevDirection.contains(direction.oppositeDirection) }
            } else {
              existingNeighbors.filter { case (direction, coordinate) => !head.prevDirection.contains(direction) && !head.prevDirection.contains(direction.oppositeDirection) }
            }
          ...
          if (head.straightStepsSoFar >= 4 && head.coordinate == end)
            smallestKnownHeatLosses.toMap
          else
            getMinHeatLossMap
        }
