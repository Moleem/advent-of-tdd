...
object OtherGalaxyMapParser extends ContentParser[ Map[Int, (Int, Int)]] {
  override def parse(content: String):  Map[Int, (Int, Int)] = {
    ...
    galaxies.map { case (id, (row, col)) =>
      val numberOfEmptyRowsBefore = 1000000 * getNumberOfEmptyRowsBefore(row)
      val numberOfEmptyColdBefore = 1000000 * getNumberOfEmptyColsBefore(col)
