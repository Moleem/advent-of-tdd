...
class DummyTask2ParserSpec extends AnyFlatSpec with Matchers with TableDrivenPropertyChecks {

  behavior of "DummyTask2Parser"
  ...
  forAll(normalScenarios) { (scenarioName: String, expectedOutput: Seq[DummyRecord], input: String) =>
    ...
    it should s"work correctly in scenario [$scenarioName]" in {
      DummyTask2Parser.parse(input) shouldBe expectedOutput
    }
